...
def _transform_mongodb_filter_condition(condition: str) -> str:
...
def _transform_mongodb_filter_operator(operator: str) -> str:
...
def _to_mongodb_filter(standard_filters: MetadataFilters) -> Dict:
    ...
    condition = standard_filters.condition or "or"
    condition = _transform_mongodb_filter_condition(condition)
    filters_list = []
    ...
    if standard_filters.filters:
        for filter_ in standard_filters.filters:
            filters_list.append(
                {
                    filter_.key: {_transform_mongodb_filter_operator(filter_.operator): filter_.value}
                }
            )
    else:
        return {}
    final_filer = {
        condition: filters_list
    }
    logger.debug(f"mongodb_filter: \n\n{final_filer}\n\n")  # Update By Chitransh
    print(f"mongodb_filter: \n\n{final_filer}\n\n")  # Update By Chitransh
